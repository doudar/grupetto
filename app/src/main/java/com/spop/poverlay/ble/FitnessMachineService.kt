...
class FitnessMachineService(
    ...
    override fun onCharacteristicWriteRequest(
        ...
        if (characteristic.uuid == FitnessMachineConstants.ControlPointUUID) {
            handleControlPointWrite(value)
            // FTMS mandates indications for Control Point, addressed to the writing client
    ...
    override fun onDirConCharacteristicWrite(
        ...
        if (characteristic.uuid != FitnessMachineConstants.ControlPointUUID) return false
        handleControlPointWrite(value)
        return true
    }

    /**
     * Applies a Control Point procedure and publishes the Response Code. Shared by the GATT and
     * DirCon write paths, so it must not depend on a [BluetoothDevice]; the GATT path adds its own
     * per-client indication afterwards.
     */
    private fun handleControlPointWrite(value: ByteArray?) {
        // Parse opcode
